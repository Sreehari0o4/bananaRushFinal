...
# ----------------------------
# Load animated finger sprite
# ----------------------------

finger_frames = []
for filename in sorted(glob.glob("assets/s*.png")):  # loads s1.png, s2.png, s3.png
    frame = pygame.image.load(filename).convert_alpha()
    finger_frames.append(frame)

current_frame = 0
frame_delay = 5   # ticks each frame stays
frame_counter = 0

# Draw the finger sprite at given coordinates
def draw_finger_tip(x, y):
    global current_frame, frame_counter
    if finger_frames:
        # Cycle through frames
        frame_counter += 1
        if frame_counter >= frame_delay:
            current_frame = (current_frame + 1) % len(finger_frames)
            frame_counter = 0

        frame = finger_frames[current_frame]
        screen.blit(frame, (x - frame.get_width() // 2, y - frame.get_height() // 2))
    else:
        # fallback red circle if images missing
...
while running:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            if is_index_finger_up(lm):
                ...
                hand_pointing = True
                draw_finger_tip(tip_x, tip_y)  # Use the sprite here
    ...
    if monkey_tip:
        ...
        # Draw animated finger sprite instead of red glowing dot
        draw_finger_tip(monkey_tip[0], monkey_tip[1])
